...
namespace BAP.Web.Areas.Administration.Controllers
{
    ...
    public class CustomerOrdersController : BaseController<CustomerOrder>
    {
        ...
        public ActionResult Workflow(int id, int? actionId, int? transitionId, string transitionComment)
        {
            ...
            if (workflow != null)
            {
                ...
                if (actionId != null)
                {
                    //TODO: need to pass attributes values as JSON and receive it from action
                    var jsonText = workflowBl.DoAction(workflow, actionId.Value, null);
                    if (jsonText == null)
