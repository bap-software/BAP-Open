...
namespace BAP.Web.Controllers
{
    ...
    public class AccountController : BaseController<OrganizationUser>
    {
        ...
        public AccountController(ILogger logger, IConfigHelper configHelper, ILookupEngine lookupEngine, IAuthorizationContext context) :
           base(logger, configHelper, lookupEngine, context, new AuthorizationHelper<OrganizationUser>(configHelper, context))
        {            
        }

        public AccountController(ApplicationUserManager userManager, ApplicationSignInManager signInManager, ILogger logger, IConfigHelper configHelper, ILookupEngine lookupEngine, IAuthorizationContext context) :
            base(logger, configHelper, lookupEngine, context, new AuthorizationHelper<OrganizationUser>(configHelper, context))
        {
        ...
        {
            var regType = Request.QueryString["reg-type"];
        ...
        public async Task<ActionResult> Register(RegisterViewModel model, string returnUrl = "")
        {
            ...
            if(model.AgreeTnC != "on")
            {
                ModelState.AddModelError("", "You have to agree to Terms and Conditions before to proceed.");
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var regEngine = new UserRegistrator(_configHelper, _authContext, _logger);
                    ...
                    RegistrationResponse response = regEngine.RegisterCompany(regModel);
                    ...
                    var callbackUrl = Url.Action("ConfirmEmail", "Account", new { userId = user.Id, code = code, returnUrl = returnUrl }, protocol: Request.Url.Scheme);
