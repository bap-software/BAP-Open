...
namespace BAP.Web.Areas.Administration.Controllers
{
    ...
    {
        public MessagesController(ILogger logger, IConfigHelper configHelper, IFileProcessor fileProc, ILookupEngine lookupEngine, IAuthorizationContext context, IMailer mailer) :
            base(logger, configHelper, lookupEngine, context, new AuthorizationHelper<Message>(configHelper, context), mailer, fileProc)
        {            
        }
        ...
        public async Task<ActionResult> Resend(int id)
        {
            ...
            await _mailer.SendEmailAsync(message.FromAddress, message.ToAddress, "RE:" + message.Subject, message.Body, message.IsHtml);
            _logger.LogText("MessagesController", "Resend", $"Message has been resent to {message.ToAddress}");
