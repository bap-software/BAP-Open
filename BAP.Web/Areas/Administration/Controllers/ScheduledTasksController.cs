...
namespace BAP.Web.Areas.Administration.Controllers
{
    ...
    public class ScheduledTasksController : BaseController<ScheduledTask>
    {
        ...
        public ActionResult RunTask(int? id)
        {
            ...
            var scheduledTask = _bl.GetScheduledTaskById(id.Value);   
            ...
            try
            {
                taskEngine.ExecuteTask(scheduledTask);
            }
            catch (Exception ex)
            {
                _logger.LogException(nameof(ScheduledTasksController), nameof(RunTask), ex);
